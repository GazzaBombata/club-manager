@php
  $companies = [
    [
      ['name' => 'Fisiospecialist', 'logo' => 'fisiospecialist-removebg-preview.png'],
      ['name' => 'Manus Brescia', 'logo' => 'manus_brescia-removebg-preview.png'],
      ['name' => 'Palestra Mustang', 'logo' => 'Logo_palestra-mustang.png'],
    ],
    [
...
<x-tailwind.container class="pb-16 pt-20 text-center lg:pt-32">
  <h1 class="mx-auto max-w-4xl font-display text-5xl font-medium tracking-tight text-slate-900 sm:text-7xl">
    ...
    <span class="relative whitespace-nowrap text-coral-600">
      ...
      <span class="relative"> semplice</span>
    </span>
    per il tuo club.
  </h1>
  ...
  <div class="mt-10 flex justify-center gap-x-6">
        ...
        >
          <svg
            aria-hidden="true"
            class="h-3 w-3 flex-none fill-coral-600 group-active:fill-current"
          >
            <path d="m9.997 6.91-7.583 3.447A1 1 0 0 1 1 9.447V2.553a1 1 0 0 1 1.414-.91L9.997 5.09c.782.355.782 1.465 0 1.82Z" />
          </svg>
          <span class="ml-3">Guarda il video</span>
        </x-tailwind.button> --}}
  </div>
{{--  <div class="mt-36 lg:mt-44">
        <p class="font-display text-base text-slate-900">
          Alcuni dei nostri professionisti affiliati:
        </p>
        <ul
          role="list"
            ...
            <li>
              <ul role="list" class="flex flex-col items-center gap-y-8 sm:flex-row sm:gap-x-12 sm:gap-y-0">
                @foreach($company as $item)
                  <li class="flex flex-col items-center justify-items-center space-y-2">
                    <img src="{{ asset('images/logos/' . $item['logo'] ) }}" alt="{{ $item['name'] }}" class="w-full md:h-[145px]" />
                      <p class="font-display text-base text-slate-600">
                          {{ $item['name'] }}
                      </p>
                  </li>
                @endforeach
